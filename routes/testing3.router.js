...
router.post('/updateuser', async (req, res) => {
	const client = await pool.connect();
	const { email, firstname, lastname, country_code, country, education, branch } = req.body;
	try {
		if (!firstname || !lastname || !country_code || !country || !education || !branch) {
			return sendErrorMessage(res, 'Bad Request');
		}
		...
		WHERE email = $1
		RETURNING *`;
		var values = [email, firstname, lastname, country_code, country, education, branch];
		var { rows } = await client.query(query, values);
		if (rows.length == 1) {
			return sendOkResponse(res, []);
		} else {
